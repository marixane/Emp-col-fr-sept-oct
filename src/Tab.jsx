...
const GROUP_TITLES = ['1 AC', '2 AC', '3 AC'];
...
const MANDATORY_EVENTS = [
  ...
  { start: '10/07', end: '10/07', label: 'Collège', text: 'Signature du Procès-verbal de sortie', type: 'exam' },
];
const EXAM_EVENTS = [
...
const levelGroupTitleStyle = { marginBottom: '8px', color: '#111827', fontSize: '18px', fontWeight: 900, textAlign: 'center', textTransform: 'uppercase', letterSpacing: '0.3px' };
...
const getClassLevel = (className) => {
  ...
  if (normalized.startsWith('2AC')) return '2 AC';
  if (normalized.startsWith('3AC')) return '3 AC';
  return '1 AC';
};
const getGroupIndex = (className) => {
  const level = getClassLevel(className);
  return level === '2 AC' ? 1 : level === '3 AC' ? 2 : 0;
};
const getMandatoryEventStart = (monthDate) => {
  const events = MANDATORY_EVENTS.filter((event) => event.start === monthDate);
  const collegeExam = events.find((event) => event.type === 'exam' && event.label === 'Collège');
  return collegeExam ? [collegeExam] : events;
};
...
export default function Tab({ onClassGroupsChange }) {
  ...
  useEffect(() => {
    const handleCompactChange = (event) => setCompactTimetable(Boolean(event.detail?.enabled));
    window.addEventListener('cahier-compact-timetable-change', handleCompactChange);
    return () => window.removeEventListener('cahier-compact-timetable-change', handleCompactChange);
  }, []);
  ...
  const previousExpansionIndex = (hourIndex) => {
    const previousIndex = hourIndex - 1;
    return compactTimetable && (previousIndex === 4 || previousIndex === 5) ? 3 : previousIndex;
  };
  const nextExpansionIndex = (hourIndex, span) => {
    const nextIndex = hourIndex + span;
    return compactTimetable && (nextIndex === 4 || nextIndex === 5) ? 6 : nextIndex;
  };
  const canExtendLeft = (row, hourIndex) => {
    const targetIndex = previousExpansionIndex(hourIndex);
    return targetIndex >= 0 && Boolean(normalizeCell(row.cells[hours[hourIndex]]).text.trim()) && !normalizeCell(row.cells[hours[targetIndex]]).hidden && !normalizeCell(row.cells[hours[targetIndex]]).text.trim();
  };
      ...
      {groupedHomeworkPages.flatMap((group, groupIndex) => [
        ...
        ...group.pages.map((pageEntries, pageIndex) => <div className="a4-page cahier-page homework-page" key={`homework-page-${groupIndex}-${pageIndex}`} style={{ '--group-color': group.color, position: 'relative', paddingTop: '60px' }}>
          ...
          {pageEntries.map((entry) => <section className={`homework-entry ${entry.isExam ? 'cahier-exam-entry' : ''} ${entry.isHoliday ? 'cahier-extra-holiday-entry' : ''} ${entry.isMidYearHoliday ? 'cahier-midyear-holiday-entry' : ''}`} key={`${group.title}-${entry.date}-${entry.eventKey || entry.text}`} style={{ '--homework-color': entry.isMidYearHoliday ? '#16a34a' : entry.color }}>
            <div className="homework-date" data-assignment-week-label={assignmentWeekLabels.get(entry)} contentEditable suppressContentEditableWarning onKeyDown={validateOnEnter}>{entry.date}</div>
